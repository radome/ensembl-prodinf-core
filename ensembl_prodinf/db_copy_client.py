...
class DbCopyClient(RestClient):
    ...
    def submit_job(self, **kwargs):
        ...
        """
        source_db_uri = kwargs.get('source_db_uri', '')
        target_db_uri = kwargs.get('target_db_uri', '')
        only_tables = kwargs.get('only_tables', '')
        skip_tables = kwargs.get('skip_tables', '')
        ...
        if only_tables:
            if (not re.search(r"^([^ ]+){1}$", only_tables) or not re.search(r"^([^ ]+){1}(,){1}([^ ]+){1}$",
                                                                             only_tables)):
                raise ValueError("List of tables need to be comma separated, eg: table1,table2,table3")
        if skip_tables:
            if (not re.search(r"^([^ ]+){1}$", skip_tables) or not re.search(r"^([^ ]+){1}(,){1}([^ ]+){1}$",
                                                                             skip_tables)):
                raise ValueError("List of tables need to be comma separated, eg: table1,table2,table3")
    ...
    if args.action == 'submit':
        ...
        if args.input_file is None:
            ...
            job_id = client.submit_job(args.source_db_uri, args.target_db_uri, args.only_tables, args.skip_tables,
                                       args.update, args.drop, args.email)
