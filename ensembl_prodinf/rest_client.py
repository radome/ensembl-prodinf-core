...
from ensembl_prodinf.utils import assert_http_uri
...
def retry_requests(test_func):
    ...
    :param test_func: Api client function to call
    """

    def retry_call_api(*func_args, **kwargs):
        retry = 1
        max_retry = 6
        while retry <= max_retry:
            try:
                return test_func(*func_args, **kwargs)
...
class RestClient(object):
    ...
    def retrieve_job_email(self, job_id):
        """
        Retrieve information on a job using the special format "email" which renders the supplied job in a format
    ...
    def print_job(self, a_job, print_results=False, print_input=False):
        """
