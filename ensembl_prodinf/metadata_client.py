...
class MetadataClient(RestClient):
    ...
    def submit_job(self, **kwargs):
        ...
        e_release = kwargs.get('e_release', "")
        eg_release = kwargs.get('eg_release', "")
        release_date = kwargs.get('release_date', "")
        current_release = kwargs.get('current_release', "")
        email = kwargs.get('email', "")
        comment = kwargs.get('comment', "")
        source = kwargs.get('source', "")
        email_notification = kwargs.get('email_notification', "")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--action', help='Action to take',
                        choices=['submit', 'retrieve', 'list', 'delete', 'email'], required=True)
    ...
    if args.action == 'submit':
        if args.input_file is None and args.email_notification is None:
            ...
            logging.info("Submitting " + args.database_uri + " for metadata load")
            job_id = client.submit_job(args.database_uri, args.e_release, args.eg_release, args.release_date,
                                       args.current_release, args.email, args.update_type, args.comment, args.source,
            ...
            for line in args.input_file:
                ...
                job_id = client.submit_job(uris[0], args.e_release, args.eg_release, args.release_date,
                                           args.current_release, args.email, args.update_type, args.comment,
                                           args.source, None)
                logging.info('Job submitted with ID ' + str(job_id))
