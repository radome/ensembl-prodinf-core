...
species_pattern = re.compile(
    "(.*[a-z0-9])_(core|rnaseq|cdna|otherfeatures|variation|funcgen)_?[0-9]*?_([0-9]*)_([0-9a-z]*)")
...
def handover_database(spec):
    ...
    * progress_complete - Total number of task completed
    ...
    if db_type not in db_types_list:
        get_logger().error("Handover failed, " + spec[
            'src_uri'] + " has been handed over after deadline. Please contact the Production team")
        raise ValueError(
            spec['src_uri'] + " has been handed over after the deadline. Please contact the Production team")
...
def submit_hc(spec, groups, compara_uri, staging_uri):
    ...
    try:
        hc_job_id = hc_client.submit_job(spec['src_uri'], cfg.production_uri, compara_uri, staging_uri, cfg.live_uri,
                                         None, groups, cfg.data_files_path, None, spec['handover_token'])
...
def submit_metadata_update(spec):
    """Submit the source database for copying to the target. Returns a celery job identifier."""
    try:
        metadata_job_id = metadata_client.submit_job(spec['tgt_uri'], None, None, None, None, spec['contact'],
                                                     spec['comment'], 'Handover', None)
    except Exception as e:
...
def process_db_metadata(self, metadata_job_id, spec):
    ...
    * if failure, flag error using email"""
    ...
    if result['status'] == 'failed':
        get_logger().info(
            "Metadata load failed, please see " + cfg.meta_uri + 'jobs/' + str(metadata_job_id) + '?format=failures')
        msg = """
...
def drop_old_assembly_databases(old_assembly_db_list, staging_uri, tgt_uri):
    ...
    if tgt_url.database in old_assembly_db_list:
        get_logger().debug(
            "The assembly has been updated but the new database " + str(tgt_url.database) + " is the same as old one")
    else:
